...
player1.style.backgroundColor = "rgb(235, 82, 255)";
...
var handleClick = function (event) {
    ...
    var arrayInsertIdxPosition = Number(
        event.target.getAttribute("data-ArrayIdx")
    );
    ...
    if (playerOneTurn === true) {
        ...
        player2.style.backgroundColor = "rgb(235, 82, 255)";
...
var checkWin = function (potentialWinner) {
    ...
    if (potentialWinner === answerX) {
        ...
        player1ScoreCounter.textContent =
            Number(player1ScoreCounter.textContent) + 1;
        ...
        player2ScoreCounter.textContent =
            Number(player2ScoreCounter.textContent) + 1;
        // update back to player 1 to start. Remove this if you want alternating player starts
        whichPlayerCounter = 0;
    }
};
...
var obtainRemainingPositions = function () {
    ...
    var randomIndexPositionToInsertAZero = getRandomIndexPosition(
        0,
        availableIndexPosittions.length - 1
    );

    // take the random index position that was calculated and used the string's value number to update the grid array
    updateGameGridArray(
        availableIndexPosittions[randomIndexPositionToInsertAZero],
        "0"
    );
    ...
    for (var i = 0; i < boxElements.length; i++) {
        if (
            boxElements[i].getAttribute("data-ArrayIdx") ==
            availableIndexPosittions[randomIndexPositionToInsertAZero]
        ) {
